...
export class OpenCodeLogWatcher extends EventEmitter<LogWatcherEvents> {
  ...
  private parseLine(line: string): void {
    ...
    console.log('[LogWatcher] Parsing error line:', line);
    ...
    for (const { pattern, extract } of ERROR_PATTERNS) {
      ...
      if (match) {
        ...
        if (this.seenErrors.has(errorKey)) {
          console.log('[LogWatcher] Skipping duplicate error:', errorKey);
          continue;
        }
        ...
        console.log('[LogWatcher] Error details:', JSON.stringify(error, null, 2));
        this.emit('error', error);
        return;
      }
    }

    console.log('[LogWatcher] ERROR line did not match any pattern:', line);
  }

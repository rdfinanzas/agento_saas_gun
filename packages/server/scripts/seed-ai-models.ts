/**
 * Seed para proveedores y modelos de IA
 * Basado en el backend legacy con Prisma
 * Uso: bun run scripts/seed-ai-models.ts
 */

import { db } from "../src/db"
import { aiProviders, aiModels } from "../src/db/schema"
  ...
  try {
    ...
    for (const providerData of PROVIDERS) {
      ...
      const [provider] = await db.insert(aiProviders).values({
        ...
        isActive: providerData.isActive ? 'true' : 'false',
        isDefault: providerData.isDefault ? 'true' : 'false',
      ...
      for (const modelData of providerData.models) {
        await db.insert(aiModels).values({
          ...
          isActive: modelData.isActive ? 'true' : 'false',
          maxTokens: modelData.maxTokens?.toString(),
          supportsVision: modelData.supportsVision ? 'true' : 'false',
          supportsTools: modelData.supportsTools ? 'true' : 'false',
          supportsStreaming: modelData.supportsStreaming ? 'true' : 'false',
          costPer1kTokens: modelData.costPer1kTokens?.toString(),
          createdAt: now,
          updatedAt: now,
        })
    ...
    let totalModels = 0
    providers.forEach((p) => {
      const status = p.isActive === 'true' ? '✓' : '✗'
      const def = p.isDefault === 'true' ? '[DEFAULT]' : ''
